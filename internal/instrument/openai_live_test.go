...
// time. Skipped unless OPENAI_BASE_URL and a model (OPENAI_MODEL or
// OPENAI_MODEL_LIST) are set.
//
...
//	export OPENAI_API_KEY=<your key>                  # optional if unauthenticated
...
func TestOpenAI_LiveEndpoint(t *testing.T) {
	...
	if base == "" || len(models) == 0 {
		t.Skip("set OPENAI_BASE_URL and OPENAI_MODEL or OPENAI_MODEL_LIST (and OPENAI_API_KEY if required) to run the live check")
	}
...
// trips on. Runs once per model in OPENAI_MODEL_LIST/OPENAI_MODEL.
