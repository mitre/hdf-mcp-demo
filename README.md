...
| `cross-format-high-count` | high-severity-or-above findings across the gosec + ZAP + grype scans together | objective (60) | The Heimdall workflow: one question over three *unlike* severity vocabularies (gosec HIGH/MEDIUM/LOW, ZAP numeric riskcodes, grype Critical…Unknown). The raw arm must discover and reconcile all three itself; the HDF arm filters one normalized impact scale. One of three multi-source questions. |
...
| `grype-fixed-vulns` | distinct vulnerability IDs fixed between two scans of the same image | objective (5) | A real temporal pair: grype of alpine:3.11 and alpine:3.12, one grype version and DB (see PROVENANCE). Distinct-ID level on purpose — instance-level diffs report churn for CVEs that persist across package version bumps. |
| `sbom-vuln-free-packages` | SBOM packages with no vulnerability matches | raw-only (8) | The bank's genuine Class D. The syft SBOM covers the same image grype scanned, but `hdf system create` carries the SBOM as a *reference* (`boms[].ref`), not an embedded inventory — the join key never enters the HDF view. The raw arm must join two files; the HDF arm is out of remit, measured on hallucinate-vs-abstain. |
...
What's deliberately *not* here: prose questions ("summarize the risk posture") that would need an LLM judge. The bank's two honest HDF-loses cases arise from real carriage decisions, not fabricated gaps: `grype-related-vulns` (the field survives conversion but not the read surface) and `sbom-vuln-free-packages` (the SBOM inventory is referenced, not embedded, so the join key never reaches the HDF view at all).
...
- **Local inference is slow.** A 30B model can take ~30 minutes for the fourteen questions; the whole ladder is an afternoon. The InSpec pair reads a 1.2MB document, so the raw arm works hardest there. Timeouts scale automatically with the model count, and a model that stalls is skipped rather than taking the run down with it.
- **N is small and one run is noisy.** Fourteen questions at `-repeat 1` is directional at best. Use `-repeat 3` before believing any single number.
